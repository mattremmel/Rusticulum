...
docker compose -f docker-compose.ifac-reject-test.yml up -d --build 2>&1 | tee "$LOG_FILE"

echo "=== Waiting for IFAC rejection test (60s — expecting silence) ==="
TIMEOUT=60

# Wait for Python test script to finish
PYTHON_CONTAINER=$(docker compose -f docker-compose.ifac-reject-test.yml ps -q python-rns 2>/dev/null)
...
# Wait for Rust node
RUST_CONTAINER=$(docker compose -f docker-compose.ifac-reject-test.yml ps -q rust-node 2>/dev/null)
if [ -n "$RUST_CONTAINER" ]; then
    timeout 30 docker wait "$RUST_CONTAINER" 2>/dev/null || true
fi

echo "=== Collecting logs ==="
docker compose -f docker-compose.ifac-reject-test.yml logs >> "$LOG_FILE" 2>&1
...
echo "=== Tearing down ==="
docker compose -f docker-compose.ifac-reject-test.yml down -v
