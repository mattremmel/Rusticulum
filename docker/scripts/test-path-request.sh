...
docker compose -f docker-compose.yml -f docker-compose.path-request-test.yml up -d --build 2>&1 | tee "$LOG_FILE"

echo "=== Waiting for python-rns test script to complete (120s timeout) ==="
TIMEOUT=120

PYTHON_CONTAINER=$(docker compose -f docker-compose.yml -f docker-compose.path-request-test.yml ps -q python-rns 2>/dev/null)
...
RUST_CONTAINER=$(docker compose -f docker-compose.yml -f docker-compose.path-request-test.yml ps -q rust-node 2>/dev/null)
if [ -n "$RUST_CONTAINER" ]; then
    timeout 30 docker wait "$RUST_CONTAINER" 2>/dev/null || true
fi

echo "=== Collecting logs ==="
docker compose -f docker-compose.yml -f docker-compose.path-request-test.yml logs >> "$LOG_FILE" 2>&1
...
echo "=== Tearing down ==="
docker compose -f docker-compose.yml -f docker-compose.path-request-test.yml down -v
