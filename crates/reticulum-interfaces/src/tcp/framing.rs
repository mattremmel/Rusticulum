//! Streaming HDLC frame accumulator for TCP byte streams.
//!
//! TCP delivers arbitrary byte chunks. This module provides a stateful buffer
//! that accumulates bytes and extracts complete HDLC-delimited frames, matching
//! the Python reference `read_loop` behavior.

use reticulum_core::constants::HEADER_MINSIZE;
use reticulum_core::framing::hdlc::{ESC, ESC_MASK, FLAG};

/// Stateful accumulator that buffers TCP stream data and extracts complete
...
impl HdlcFrameAccumulator {
    ...
    /// Feed new data from the TCP stream and extract all complete frames.
    ///
